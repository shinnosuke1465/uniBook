...
class ReportReceiptApiTest extends TestCase
{
     ...
     */
    public function test_正常に受取報告ができること(): void
    {
        // given: 出品者と購入者を作成
        ...
        // 配送中の取引を作成
        $textbookId = $this->createTextbookWithShippingDeal($seller->id, $buyer->id);
        ...
        $url = route('textbooks.deals.reportReceipt', ['textbookId' => $textbookId->value]);
        ...
        // 取引ステータスがCompleted（完了）に更新されていることを確認
     ...
     * @throws DuplicateKeyException
     */
    public function test_出品者が受取報告しようとすると403エラー(): void
    {
        // given: 出品者と購入者を作成
        $seller = $this->prepareUserWithFacultyAndUniversity(
            new String255('password12345'),
     ...
     * @throws DuplicateKeyException
     */
    public function test_取引ステータスがPurchased状態では受取報告できない(): void
    {
        // given: 出品者と購入者を作成
        $seller = $this->prepareUserWithFacultyAndUniversity(
            new String255('password12345'),
     ...
     * @throws AuthenticationException
     */
    public function test_存在しない教科書IDの場合は404エラー(): void
    {
        // given: 購入者を作成
        $buyer = $this->prepareUserWithFacultyAndUniversity(
            new String255('password12345'),
        ...
        $url = route('textbooks.deals.reportReceipt', ['textbookId' => $nonExistentId]);

        // when: 存在しない教科書IDでリクエスト
        $response = $this->postJson($url, [], [
            'Authorization' => 'Bearer ' . $token->token,
        ]);
    ...
    private function createTextbookWithPurchasedDeal(UserId $sellerId, UserId $buyerId): TextbookId
    {
        ...
        $textbook = TestTextbookFactory::create(
            id: new TextbookId('f1f6d6cb-4f7a-4970-8b2a-9c1c1c3c4a78'),
     ...
     * @throws DuplicateKeyException
     */
    private function createTextbookWithShippingDeal(UserId $sellerId, UserId $buyerId): TextbookId
    {
        // 出品者の情報を取得
        $seller = $this->userRepository->findById($sellerId);
        if ($seller === null) {
        ...
        $textbook = TestTextbookFactory::create(
            ...
            name: new String255('配送中教科書'),
            price: new Price(3000),
            description: new Text('これは配送中の教科書です。'),
            imageIdList: new ImageIdList([$image->id]),
            universityId: $seller->universityId,
            facultyId: $seller->facultyId,
            conditionType: ConditionType::NEW
