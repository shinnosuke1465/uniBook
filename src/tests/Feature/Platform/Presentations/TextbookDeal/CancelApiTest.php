...
class CancelApiTest extends TestCase
{
     ...
     */
    public function test_正常に出品キャンセルができること(): void
    {
        // given: 出品者を作成
        ...
        $url = route('textbooks.deals.cancel', ['textbookId' => $textbookId->value]);

        // when: 出品キャンセルAPIを呼び出す
    ...
    public function test_存在しない教科書IDの場合は404エラー(): void
    {
        ...
        $url = route('textbooks.deals.cancel', ['textbookId' => $nonExistentId]);
     ...
     * @throws DuplicateKeyException
     */
    private function createTextbookWithoutDeal(UserId $sellerId): TextbookId
    {
        // 出品者の情報を取得
        $seller = $this->userRepository->findById($sellerId);
        if ($seller === null) {
        ...
        $image = TestImageFactory::create(
            path: new String255('/path/to/test_image3.jpg'),
            type: new String255('jpg')
        );
        $this->imageRepository->insert($image);

        $textbook = TestTextbookFactory::create(
            id: new TextbookId('e1f6d6cb-4f7a-4970-8b2a-9c1c1c3c4a80'),
            name: new String255('取引なし教科書'),
            price: new Price(1000),
            description: new Text('これは取引が存在しない教科書です。'),
            imageIdList: new ImageIdList([$image->id]),
            universityId: $seller->universityId,
            facultyId: $seller->facultyId,
